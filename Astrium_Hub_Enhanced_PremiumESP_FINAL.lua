...
local function ESPProjectClamped(camera,worldPosition,padding)
    ...
    local pad=math.clamp(tonumber(padding) or 24,8,80)
    local center=Vector2.new(vp.X*.5,vp.Y*.5)
    local depth=screen.Z
    local p=Vector2.new(screen.X,screen.Y)
    if depth<=0 then
        -- A point behind the camera must be mirrored through the camera plane.
        -- This keeps the off-screen arrow pointing toward the real target.
        p=center-(p-center)
    end
    local inside=(depth>0 and onScreen==true and p.X>=0 and p.X<=vp.X and p.Y>=0 and p.Y<=vp.Y)
    if inside then return p,true,depth end
    local dir=p-center
    if dir.Magnitude<0.001 then dir=Vector2.new(0,-1) end
    local tx,ty=math.huge,math.huge
    if math.abs(dir.X)>0.0001 then
        tx=((vp.X*.5)-pad)/math.abs(dir.X)
    end
    if math.abs(dir.Y)>0.0001 then
        ty=((vp.Y*.5)-pad)/math.abs(dir.Y)
    end
    local scale=math.min(tx,ty)
    if scale==math.huge then scale=1 end
    local cp=center+dir*scale
    cp=Vector2.new(math.clamp(cp.X,pad,vp.X-pad),math.clamp(cp.Y,pad,vp.Y-pad))
    return cp,false,depth
end
...
local function ESPBindPlayer(player)
    ...
    table.insert(ESPPlayerConnections[player],ESPTrackConnection(player.CharacterRemoving:Connect(function(character)
        local e=ESPEntries[player]
        if e and e.character==character then
            e.character=nil
...
local function ESPProjectBounds(camera,character)
    ...
    local vp=camera.ViewportSize
    minX=math.clamp(minX,-160,vp.X+160)
    maxX=math.clamp(maxX,-160,vp.X+160)
    minY=math.clamp(minY,-160,vp.Y+160)
    maxY=math.clamp(maxY,-160,vp.Y+160)
        ...
        for _,chain in ipairs(chains) do
            ...
            if line then
                ...
                if pa and pb and pa:IsA("BasePart") and pb:IsA("BasePart") then
                    local a,ao=ESPProject(camera,pa.Position)
                    local b,bo=ESPProject(camera,pb.Position)
                    if a and b and (ao or bo) then
