...
local function MakeESPLine(parent,a,b,thickness)
...
local function CreateESP(player)
    ...
    Corner(card,9)
    Outline(card,Color3.fromRGB(70,74,84))
    ...
    local dist=New("TextLabel",{
        ...
        Position=UDim2.fromOffset(119,7),
        BackgroundTransparency=1,
    ...
    -- Legacy box/tracer objects are intentionally kept as lightweight compatibility
    -- elements and remain hidden unless explicitly enabled by the user.
    local legacyBox={}
    for i=1,8 do
        legacyBox[i]=New("Frame",{BackgroundColor3=GetESPColor(player),BorderSizePixel=0,Visible=false,ZIndex=53},container)
    end
...
local function UpdateLegacyBox(e,minX,minY,maxX,maxY,color)
    local seg=e.legacyBox
    local width=maxX-minX
    local height=maxY-minY
    local cw=math.clamp(width*.25,10,30)
    local ch=math.clamp(height*.20,10,30)
    local defs={
        {UDim2.fromOffset(0,0),UDim2.fromOffset(cw,2)},
        {UDim2.fromOffset(0,0),UDim2.fromOffset(2,ch)},
        {UDim2.new(1,0,0,0),UDim2.fromOffset(cw,2),Vector2.new(1,0)},
        {UDim2.new(1,0,0,0),UDim2.fromOffset(2,ch),Vector2.new(1,0)},
        {UDim2.new(0,0,1,0),UDim2.fromOffset(cw,2),Vector2.new(0,1)},
        {UDim2.new(0,0,1,0),UDim2.fromOffset(2,ch),Vector2.new(0,1)},
        {UDim2.new(1,0,1,0),UDim2.fromOffset(cw,2),Vector2.new(1,1)},
        {UDim2.new(1,0,1,0),UDim2.fromOffset(2,ch),Vector2.new(1,1)},
    }
    for i,d in ipairs(defs) do
        local v=seg[i]
        v.Position=d[1]
        v.Size=d[2]
        v.AnchorPoint=d[3] or Vector2.new(0,0)
        v.BackgroundColor3=color
        v.Visible=Config.ESPBoxes
    end
end

local function UpdateTracerLegacy(tracer,point,color)
    if not Config.ESPTracers then tracer.Visible=false return end
    local cam=workspace.CurrentCamera
    ...
    tracer.Size=UDim2.fromOffset(1.5,len)
                ...
                if distWorld<=Config.ESPMaxDistance then
                    ...
                    if e then
                        ...
                        if onScreen and rootZ>0 then
                            ...
                            e.tracer.Visible=false
                            if Config.ESPOffscreen and rootZ>0 then
                                local margin=38
                                local clamped=Vector2.new(math.clamp(root2d.X,margin,view.X-margin),math.clamp(root2d.Y,margin,view.Y-margin))
                                local d=Vector2.new(root2d.X-center.X,root2d.Y-center.Y)
                                if d.Magnitude>0.01 then
                                    local angle=math.deg(math.atan2(d.Y,d.X))+90
                                    e.arrow.Position=UDim2.fromOffset(clamped.X,clamped.Y)
                                    e.arrow.Rotation=angle
