...
-- Central lifecycle registry. Old global listeners are disconnected before a
-- re-execution creates new ones, preventing stacked callbacks.
local Env = _G
pcall(function()
    if type(getgenv) == "function" then Env = getgenv() end
end)
Env.AstriumHubConnections = Env.AstriumHubConnections or {}
for _,connection in ipairs(Env.AstriumHubConnections) do
    pcall(function() connection:Disconnect() end)
end
table.clear(Env.AstriumHubConnections)
local function Track(signal, callback)
    local connection = signal:Connect(callback)
    table.insert(Env.AstriumHubConnections, connection)
    return connection
end

pcall(function()
    ContextActionService:UnbindAction("AstriumHub_OpenClose")
...
local Config = {
    ...
    AimTargetMode = "ClosestToCrosshair",
...
local function ApplySnapshot(snapshot)
    ...
    -- AimSticky is the canonical target-lock setting. Migrate legacy profiles
    -- that only stored AimTargetLock.
    local stickySeen = snapshot.AimSticky ~= nil
    local legacyLock = snapshot.AimTargetLock
    ...
    if not stickySeen and typeof(legacyLock) == "boolean" then
        Config.AimSticky = legacyLock
    end
    Config.AimTargetLock = Config.AimSticky -- legacy compatibility mirror
end
...
local function BindCamera()
    ...
    Resize()
    camConn=Track(cam:GetPropertyChangedSignal("ViewportSize"),Resize)
end
BindCamera()
local currentCamConn=Track(workspace:GetPropertyChangedSignal("CurrentCamera"),function() task.defer(BindCamera) end)
...
Track(UserInputService.InputChanged,function(i)
...
Track(UserInputService.InputEnded,function(i)
...
local function Slider(p,title,desc,min,max,step,get,set)
    ...
    Track(UserInputService.InputChanged,function(i) if slide and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then setX(i.Position.X); refresh() end end)
    Track(UserInputService.InputEnded,function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then slide=false; SaveProfile(Config.ActiveProfile) end end)
    refresh(); table.insert(Refreshers,refresh); return refresh
end
...
Action(Pages.Combat,"Target Priority",function()
    ...
    local modes={"ClosestToCrosshair","ClosestToPlayer","LowestHealth"}
    local i=table.find(modes,Config.AimTargetMode) or 1
    Config.AimTargetMode=modes[(i % #modes)+1]
    ResetAimState()
    SaveProfile(Config.ActiveProfile)
end,C.Accent)
Toggle(Pages.Combat,"Target Lock","Keeps one valid target until it becomes invalid.",function() return Config.AimSticky end,function(v) Config.AimSticky=v; Config.AimTargetLock=v; ResetAimState() end)
...
Info(Pages.Visuals,"Performance","ESP 2.0 updates compact screen-space elements only and avoids full-workspace scans.",C.Accent)
...
Action(Pages.Settings,"Reset features","Turn all optional Astrium features off and restore captured client state.",function()
    local defaults = {
        Aimbot=false, ESP=false, Speed=false, NoClip=false, Invisibility=false,
        FullBright=false, NoFog=false, Crosshair=false, InfiniteJump=false,
        ThirdPerson=false, CameraBob=false, FPSCounter=false, PingCounter=false,
        Coordinates=false, LowGraphics=false, TeamCheck=false, AimSmooth=false,
        AimPrediction=false, AimSticky=false, AimDeadzone=0, AimTargetLock=false,
        AutoSprint=false, FOVKick=false, CameraShake=false, ReduceParticles=false,
        DisablePostFX=false, Saturation=0, Contrast=0, ColorBoost=0, LocalTime=false,
        ESPBoxes=false, ESPTracers=false, ESPNames=false, ESPDistance=false,
        ESPHealth=false, ESPTeamColor=false, ESPHighlight=false, ESPOffscreen=false,
        ESPHeadDot=false,
    }
    for key,value in pairs(defaults) do if Config[key]~=nil then Config[key]=value end end
    ResetAimState()
    Invisibility(false)
    UpdateSpeed()
    UpdateNoClip()
...
local lastAppliedCameraOffset=nil
...
local function ApplyCameraSettings(dt)
    ...
    if c~=lastCharacterForCamera then
        ...
        lastAppliedCameraOffset=nil
    end

    if h then
        -- If another camera system changed CameraOffset since Astrium last
        -- wrote it, accept that value as the new external baseline.
        if savedCameraOffset==nil then
            savedCameraOffset=h.CameraOffset
        elseif lastAppliedCameraOffset~=nil and h.CameraOffset~=lastAppliedCameraOffset then
            savedCameraOffset=h.CameraOffset
        end
        ...
        if Config.CameraBob and not Config.ThirdPerson and h.MoveDirection.Magnitude>0.05 then
            ...
            lastAppliedCameraOffset=h.CameraOffset
        else
            if savedCameraOffset~=nil then
...
local function RebuildNoClipParts(c)
    if noclipDescConn then noclipDescConn:Disconnect(); noclipDescConn=nil end
    table.clear(noclipParts)
    noclipCharacter=c
    if not c then return end
    for _,obj in ipairs(c:GetDescendants()) do
        if obj:IsA("BasePart") then table.insert(noclipParts,obj) end
    end
    noclipDescConn=Track(c.DescendantAdded,function(obj)
        if obj:IsA("BasePart") then table.insert(noclipParts,obj) end
    end)
end
...
local function IsValidAimTarget(camera,player)
    ...
    return true,targetPart,distance,screen,humanoid,root
end
...
local function GetAimbotTarget(camera)
    ...
    if Config.AimSticky and CurrentAimPlayer then
    ...
    for _,player in ipairs(Players:GetPlayers()) do
        if player~=LocalPlayer then
            local valid,targetPart,distance,screen,humanoid,root=IsValidAimTarget(camera,player)
...
local function UpdateNoClip()
    local c=Character()
    if not c then return end
    if c~=noclipCharacter then RebuildNoClipParts(c) end
    if Config.NoClip then
        for _,obj in ipairs(noclipParts) do
...
-- Centralized state reconciliation used after profile loads, resets, and
-- re-execution. Every feature is explicitly applied in both directions.
local function ApplyAllFeatureStates()
    ResetAimState()
    UpdateSpeed()
    UpdateNoClip()
...
--========================================================
-- ESP SYSTEM 2.0 (clean tactical nameplate style)
--========================================================
...
local function GetESPColor(player)
    ...
    local ok,color=pcall(function() return player.TeamColor and player.TeamColor.Color end)
    if ok and color then return color end
    return C.Accent
end


local function ProjectPoint(cam,worldPos)
    ...
    for _,plr in ipairs(Players:GetPlayers()) do
        if plr~=LocalPlayer then
            ...
            if char and hum and root and hum.Health>0 then
                ...
                if distWorld<=Config.ESPMaxDistance then
                    ...
                    if e then
                        ...
                        if onScreen and rootZ>0 then
                            ...
                            e.headDot.Visible=Config.ESPHeadDot and head ~= nil
                            e.headDot.Position=UDim2.fromOffset(head2d.X,head2d.Y)
                            e.arrow.Visible=false

                            -- Explicitly hide box/tracer unless the compatibility toggles are on.
                            if rootZ>0 and root2d.X>=0 and root2d.X<=view.X and root2d.Y>=0 and root2d.Y<=view.Y then
                                local halfW=math.clamp(24/(rootZ^.15),12,90)
                                local halfH=math.clamp(44/(rootZ^.15),20,140)
                                UpdateLegacyBox(e,root2d.X-halfW,root2d.Y-halfH,root2d.X+halfW,root2d.Y+halfH,color)
                            end
                            UpdateTracerLegacy(e.tracer,head and cam:WorldToViewportPoint(head.Position) or cam:WorldToViewportPoint(root.Position),color)
                        else
                            e.container.Visible=false
                            e.card.Visible=false
                            e.headDot.Visible=false
                            for _,v in ipairs(e.legacyBox) do v.Visible=false end
                            e.tracer.Visible=false
                            if Config.ESPOffscreen then
                                local margin=38
                                local direction=Vector2.new(root2d.X-center.X,root2d.Y-center.Y)
                                if rootZ<0 then direction=-direction end
                                if direction.Magnitude>0.01 then
                                    local halfX=view.X*.5-margin
                                    local halfY=view.Y*.5-margin
                                    local scale=math.min(halfX/math.max(math.abs(direction.X),0.001),halfY/math.max(math.abs(direction.Y),0.001))
                                    local point=center+direction*math.max(0,math.min(scale,1))
                                    point=Vector2.new(math.clamp(point.X,margin,view.X-margin),math.clamp(point.Y,margin,view.Y-margin))
...
Track(Players.PlayerAdded,HookPlayer)
...
Track(UserInputService.JumpRequest,function()
