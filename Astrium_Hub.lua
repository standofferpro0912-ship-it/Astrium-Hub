...
--========================================================
-- PREMIUM BOTTOM-RIGHT NOTIFIER
--========================================================
local NotifierGui=New("ScreenGui",{
    Name="AstriumHub_Notifications",
    ResetOnSpawn=false,
    IgnoreGuiInset=true,
    DisplayOrder=10000,
...
local NotifierHolder=New("Frame",{
    Name="Container",
...
local NotifierList=New("UIListLayout",{
...
local function Notify(title,message,duration,kind)
    title=tostring(title or "Astrium Hub")
    message=tostring(message or "")
    duration=math.max(.8,tonumber(duration) or 3)
    kind=tostring(kind or "info"):lower()

    local accent=C.Accent
    local icon=""
    local priority=0
    if kind=="success" or kind=="pass" then
        accent=C.Good; icon="✓"; priority=1
    elseif kind=="error" or kind=="fail" then
        accent=C.Bad; icon="!"; priority=3
    elseif kind=="warning" or kind=="warn" then
        accent=Color3.fromRGB(246,190,76); icon="△"; priority=2
    else
        icon="i"; priority=0
    end
    ...
    local card=New("Frame",{
        Name="Notification_"..NotifySerial,
        Size=UDim2.fromOffset(340,66),
        BackgroundColor3=C.Surface,
        BackgroundTransparency=0.03,
        BorderSizePixel=0,
        ClipsDescendants=true,
        LayoutOrder=NotifySerial,
        ZIndex=20,
    },NotifierHolder)
    Corner(card,13); Outline(card,C.Border,1,.05); Shadow(card,.62)
    ...
    local iconFrame=New("Frame",{
        ...
        BorderSizePixel=0,
        ZIndex=21,
    },card); Corner(iconFrame,9); Outline(iconFrame,accent,1,.35)
    New("TextLabel",{
    ...
    T(card,{Position=UDim2.new(0,0,0,0)},.22)
    card:SetAttribute("AstriumNotifyPriority",priority)
    table.insert(NotifyItems,card)

    while #NotifyItems>NOTIFY_MAX do
        local newPriority=card:GetAttribute("AstriumNotifyPriority") or priority
        local removeIndex=nil
        local lowestPriority=math.huge
        for i,item in ipairs(NotifyItems) do
            if item and item.Parent then
                local itemPriority=item:GetAttribute("AstriumNotifyPriority") or 0
                if itemPriority<lowestPriority then
        ...
        local candidate=NotifyItems[removeIndex]
        local candidatePriority=candidate:GetAttribute("AstriumNotifyPriority") or 0

        -- Keep higher-priority notifications visible. During the ESP diagnostic,
        -- errors must survive the flood of PASS notifications.
        if candidate==card and candidatePriority==lowestPriority then
            local higherExists=false
            for _,item in ipairs(NotifyItems) do
                if item and item~=card and item.Parent then
                    local itemPriority=item:GetAttribute("AstriumNotifyPriority") or 0
                    if itemPriority>newPriority then
                        higherExists=true
                        break
                    end
                end
            end
            if higherExists then
                table.remove(NotifyItems,removeIndex)
        ...
        if candidate and candidate.Parent then
            T(candidate,{Position=UDim2.new(1,380,0,0)},.16)
            task.delay(.18,function() if candidate then candidate:Destroy() end end)
        end
    end

    task.delay(duration,function()
        if not card or not card.Parent then return end
        T(card,{Position=UDim2.new(1,380,0,0),BackgroundTransparency=1},.18)
        task.delay(.20,function()
            if card then card:Destroy() end
            for i,v in ipairs(NotifyItems) do
                if v==card then table.remove(NotifyItems,i); break end
            end
        end)
    end)
        ...
        if item.player.Parent~=Players or i>maxRendered then
            ...
            if not ok then
                ...
                if not Env.AstriumHubESPErrorNotified[item.player] then
                    ...
                    Notify("ESP Runtime Error",item.player.Name..": "..tostring(err):sub(1,140),6,"error")
                end
            end
        end
    end
...
--========================================================
-- ASTRIUM ESP SELF-DIAGNOSTIC
--========================================================
task.spawn(function()
    task.wait(2)
    Notify("ESP DIAGNOSTIC","Starting self-test…",3,"info")
    local pass,fail,warnCount=0,0,0
    local function P(n,v)
        pass+=1
        local msg=n..(v and " | "..tostring(v) or "")
        print("[ESP PASS] "..msg)
        Notify("ESP TEST • PASS",msg,3.2,"success")
    end
    local function F(n,v)
        fail+=1
        local msg=n..(v and " | "..tostring(v) or "")
        warn("[ESP FAIL] "..msg)
        Notify("ESP TEST • FAILED",msg:sub(1,180),5,"error")
    end
    local function W(n,v)
        warnCount+=1
        local msg=n..(v and " | "..tostring(v) or "")
        warn("[ESP WARN] "..msg)
        Notify("ESP TEST • WARNING",msg:sub(1,180),4,"warning")
    end
    local function T(n,fn)
        local ok,res=xpcall(fn,function(e)return debug.traceback(tostring(e),2)end)
        if ok and res~=false then P(n,res) else F(n,res) end
        return ok,res
    end
    print("========== ASTRIUM ESP DIAGNOSTIC ==========")
    T("Config",function() assert(Config,"nil Config") return "OK" end)
    T("ESPEntries",function() assert(type(ESPEntries)=="table","not table") return "table" end)
    T("ESPOverlay",function() assert(ESPOverlay and ESPOverlay.Parent,"missing or unparented") return ESPOverlay:GetFullName() end)
    T("CurrentCamera",function() assert(workspace.CurrentCamera,"missing camera") return "OK" end)
    local others={}
    for _,plr in ipairs(Players:GetPlayers()) do if plr~=LocalPlayer then others[#others+1]=plr end end
    if #others==0 then W("Other players","join a server with another spawned player") end
    local registered=0
    for plr,entry in pairs(ESPEntries) do if plr and entry then registered+=1 end end
    T("Player registration",function() assert(#others==0 or registered>0,"players exist but no ESP entries") return registered end)
    T("Camera projection",function()
        local cam=workspace.CurrentCamera
        local pt,on=cam:WorldToViewportPoint(cam.CFrame.Position+cam.CFrame.LookVector*10)
        assert(pt and pt.Z>0,"invalid projection")
        return string.format("%.1f,%.1f,%.1f on=%s",pt.X,pt.Y,pt.Z,tostring(on))
    end)
    local target
    for _,plr in ipairs(others) do if plr.Character then target=plr break end end
    if target then
        ...
        T("Entry for "..target.Name,function() assert(entry,"missing entry") return "OK" end)
        T("Character",function() assert(char,"missing character") return char.Name end)
        T("Humanoid",function() assert(hum,"missing humanoid") return hum.Health end)
        T("Root",function() assert(root,"missing root") return root.Name end)
        T("Head",function() assert(head,"missing head") return head.Name end)
        T("GetBoundingBox",function() local cf,size=char:GetBoundingBox() assert(cf and size,"failed") return tostring(size) end)
        if entry then
            T("Highlight",function() assert(entry.highlight and entry.highlight:IsA("Highlight"),"missing") return entry.highlight:GetFullName() end)
            T("Billboard",function() assert(entry.billboard and entry.billboard:IsA("BillboardGui"),"missing") return entry.billboard:GetFullName() end)
            T("Box pool",function() assert(type(entry.boxLines)=="table" and #entry.boxLines>=12,"missing/short") return #entry.boxLines end)
            T("Skeleton pool",function() assert(type(entry.skeleton)=="table","missing") return #entry.skeleton end)
            T("Tracer",function() assert(entry.tracer,"missing") return entry.tracer:GetFullName() end)
            T("Arrow",function() assert(entry.arrow,"missing") return entry.arrow:GetFullName() end)
            T("ESPUpdateEntry execution",function()
                local cam=workspace.CurrentCamera
                local rootLocal=Character and Character():FindFirstChild("HumanoidRootPart")
                assert(rootLocal,"local root missing")
                local ok,res=xpcall(function() return ESPUpdateEntry(target,entry,cam,rootLocal) end,function(e)return debug.traceback(tostring(e),2)end)
                assert(ok,res)
                return tostring(res)
            end)
        end
    else W("Test target","no spawned player available") end
    local resultText="Passed "..pass.." • Warnings "..warnCount.." • Failed "..fail
    print("[ESP RESULT] "..resultText)
    if fail>0 then
        warn("[ESP RESULT] FAILURES FOUND - send the [ESP FAIL] lines")
        Notify("ESP DIAGNOSTIC",resultText.." — failures found",7,"error")
    elseif warnCount>0 then
        Notify("ESP DIAGNOSTIC",resultText,6,"warning")
    else
        print("[ESP RESULT] No hard failures detected")
        Notify("ESP DIAGNOSTIC",resultText.." — all checks passed",6,"success")
    end
    print("=============================================")
end)
