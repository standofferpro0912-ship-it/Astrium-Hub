...
local function Notify(title,message,duration,kind)
    ...
    elseif kind=="warning" or kind=="warn" then
        accent=Color3.fromRGB(246,190,76); icon="△"
    else
        icon="i"
    end
    ...
    while #NotifyItems>NOTIFY_MAX do
        local old=table.remove(NotifyItems,1)
        if old and old.Parent then
            T(old,{Position=UDim2.new(1,380,0,0)},.16)
            task.delay(.18,function() if old then old:Destroy() end end)
        end
    end
...
local function ESPUpdateEntry(player,entry,camera,localRoot)
    ...
    local projected,anyOn,depths=boxCF and ESPProjectCuboid(camera,boxCF,boxSize)
    if projected then
        if Config.ESP3DBoxes then
            ...
            if Config.ESPCornerBoxes then
                ...
                for i,edge in ipairs(ESPBoxEdges) do
